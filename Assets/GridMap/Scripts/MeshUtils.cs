...
public static class MeshUtils {
    ...
    public static void AddToMeshArrays(Vector3[] vertices, Vector2[] uvs, int[] triangles, int index, Vector3 pos, float rot, Vector3 baseSize, Vector2 uv00, Vector2 uv11) {
        ...
        if (skewed) {
			...
			vertices[vIndex0] = pos+GetQuaternionEuler(rot-270)*baseSize*.9f;
			vertices[vIndex1] = pos+GetQuaternionEuler(rot-180)*baseSize*.9f;
			vertices[vIndex2] = pos+GetQuaternionEuler(rot- 90)*baseSize*.9f;
			vertices[vIndex3] = pos+GetQuaternionEuler(rot-  0)*baseSize*.9f;
		}
